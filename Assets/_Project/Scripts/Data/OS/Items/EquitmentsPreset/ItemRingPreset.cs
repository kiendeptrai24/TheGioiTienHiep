...
[CreateAssetMenu(fileName = "NewRingPreset", menuName = "RPG/Items/Equipment/Ring Preset")]
public class ItemRingPreset : ItemEquipmentPreset
{
    [Header("Ring")]
    ...
    public void ResetToDefault()
    {
        ...
        itemName = $"{GetRingName(ringType)} C{level}";

        switch (ringType)
        {
            case RingType.NhanCo:
                ApplyNhanCo();
                break;

            case RingType.NhanThuyTinh:
                ApplyNhanThuyTinh();
                break;

            case RingType.NhanBachNgan:
                ApplyNhanBachNgan();
                break;
        }

#if UNITY_EDITOR
        string newName = $"Ring_{itemName}_{qualityType}";
        if (name != newName)
        {
    ...
    // Nhẫn cỏ: C1 Phàm => physicalDamage 20%
    private void ApplyNhanCo()
    {
        level = 1;
    ...
    // Nhẫn thủy tinh: C2 => physicalDamage: Phàm 30, Hoàng 40, Huyền 70
    private void ApplyNhanThuyTinh()
    {
        level = 2;
    ...
    private void ApplyNhanBachNgan()
    {
        level = 3;
    ...
    private static string GetRingName(RingType t)
    {
        return t switch
        {
            RingType.NhanCo => "Nhẫn cỏ",
            RingType.NhanThuyTinh => "Nhẫn thủy tinh",
            RingType.NhanBachNgan => "Nhẫn bạch ngân",
            _ => t.ToString()
        };
    }
