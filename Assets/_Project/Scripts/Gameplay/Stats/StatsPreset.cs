...
[CreateAssetMenu(fileName = "NewStatsPreset", menuName = "RPG/Stats/Stats Preset")]
public class StatsPreset : ScriptableObject
{
    [Header("Cultivation Realm")]
    public CultivationStage cultivationStage;
    [Header("Resources")]
    public Stat health;     // StatType.Health
    public Stat mana;       // StatType.Mana
    public Stat spirit;     // StatType.Spirit

    [Header("Offensive Stats")]
    public Stat physicalDamage; // StatType.PhysicalDamage
    public Stat magicalDamage;  // StatType.MagicalDamage
    public Stat spiritDamage;   // StatType.SpiritDamage
    public Stat critChance;     // StatType.CritChance
    public Stat critPower;      // StatType.CritPower

    [Header("Defensive Stats")]
    public Stat physicalDefense;    // StatType.PhysicalDefense
    public Stat magicalDefense;     // StatType.MagicalDefense
    public Stat spiritDefense;      // StatType.SpiritDefense
    public Stat evasion;            // StatType.Evasion
    public Stat spiritPenetration;  // StatType.SpiritPenetration
    public Stat mindPenetration;    // StatType.MindPenetration

    [Header("Speed Stats")]
    public Stat movementSpeed;  // StatType.MovementSpeed
    public Stat attackSpeed;    // StatType.AttackSpeed
    public Stat castSpeed;      // StatType.CastSpeed

    [Header("Progression Stats")]
    public Stat potential;      // StatType.Potential (điểm tiềm năng để cộng vào các core stat)
    public Stat skillPoints;    // StatType.SkillPoints
    public Stat combatPower;    // StatType.CombatPower
#if UNITY_EDITOR
    private void OnValidate()
    {
        // Không làm gì nếu chưa set stage
        // (nếu bạn muốn bắt buộc, có thể bỏ check này)
        string stageName = cultivationStage.ToString();
        if (string.IsNullOrEmpty(stageName))
            return;

        // Đặt tên asset: Stats_[TênStage]  VD: Stats_LuyenKhi_1
        string newName = $"Stats_{stageName}";

        // Chỉ rename nếu đang ở trong editor & tên khác
        if (name != newName)
        {
    ...
    // hàm dùng chung để map bảng -> Stat
        ...
        float tiemNang, float phapKyDiem, float power)
    {
        ...
        spiritPenetration = new Stat(StatType.SpiritPenetration, pvLinhThuc);
